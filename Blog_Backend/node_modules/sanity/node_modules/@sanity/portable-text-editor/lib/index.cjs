...
var debug__default = /*#__PURE__*/_interopDefaultLegacy(debug$h);
...
const dmp$3 = new DMP__namespace.diff_match_patch();
...
function validateValue(value, portableTextFeatures, keyGenerator) {
    ...
    if (value.some((blk, index) => {
        ...
        if (blk._type === portableTextFeatures.types.block.name) {
            ...
            //   return true
            // }
            const allUsedMarks = lodash.uniq(lodash.flatten(blk.children
                .filter((cld) => cld._type === portableTextFeatures.types.span.name)
                .map((cld) => cld.marks || [])));
 ...
 */
const PortableTextEditorValueContext = React.createContext(undefined);
...
function createOperationToPatches(portableTextFeatures) {
    ...
    function setNodePatch(editor, operation) {
        if (operation.path.length === 1) {
            ...
            const setNode = lodash.omitBy({ ...editor.children[operation.path[0]], ...operation.newProperties }, lodash.isUndefined);
    ...
    function insertNodePatch(editor, operation, beforeValue) {
        ...
        if (operation.path.length === 1) {
            ...
            const targetKey = operation.path[0] === 0 ? block === null || block === void 0 ? void 0 : block._key : beforeBlock === null || beforeBlock === void 0 ? void 0 : beforeBlock._key;
            if (targetKey) {
                return [
...
// eslint-disable-next-line new-cap
const dmp$2 = new DMP__namespace.diff_match_patch();
const OPERATIONS = {
...
// eslint-disable-next-line new-cap
const dmp$1 = new DMP__namespace.diff_match_patch();
function createPatchToOperations(portableTextFeatures, keyGenerator) {
    ...
    function insertPatch(editor, patch) {
        ...
        if (patch.path.length === 1) {
            ...
            slate.Transforms.insertNodes(editor, blocksToInsert, { at: [normalizedIdx] });
            if (isEmpty) {
    ...
    function setPatch(editor, patch) {
        ...
        const isTextBlock = portableTextFeatures.types.block.name === (block === null || block === void 0 ? void 0 : block._type);
        if (isTextBlock) {
        ...
        // If this is a object block, just set the whole block
            ...
            slate.Transforms.setNodes(editor, { ...block, value: newVal }, { at: [blockIndex] });
            return true;
        }
        return false;
    }
    function unsetPatch(editor, patch, previousPatch) {
        ...
        if (patch.path.length === 0) {
            ...
            slate.Transforms.insertNodes(editor, editor.createPlaceholderBlock(), { at: [0] });
            slate.Transforms.select(editor, {
                focus: { path: [0, 0], offset: 0 },
                anchor: { path: [0, 0], offset: 0 },
            });
        ...
        if (isTextBlock && patch.path[1] === 'children' && patch.path.length === 3) {
            ...
            const prevSel = editor.selection && { ...editor.selection };
            const onSamePath = lodash.isEqual((_a = editor.selection) === null || _a === void 0 ? void 0 : _a.focus.path, targetPath);
            ...
            if (prevSel && onSamePath && editor.isTextBlock(block)) {
                ...
                if (needToAdjust) {
                    const isMergeUnset = (previousPatch === null || previousPatch === void 0 ? void 0 : previousPatch.type) === 'set' &&
...
function transformOperation(editor, patch, operation) {
    ...
    if (patch.type === 'diffMatchPatch') {
        const blockIndex = editor.children.findIndex((blk) => lodash.isEqual({ _key: blk._key }, patch.path[0]));
        const block = editor.children[blockIndex];
        if (slate.Element.isElement(block) && Array.isArray(block.children)) {
            const childIndex = block.children.findIndex((child) => lodash.isEqual({ _key: child._key }, patch.path[2]));
...
function adjustBlockPath(editor, patch, operation, level) {
    const myIndex = editor.children.findIndex((blk) => lodash.isEqual({ _key: blk._key }, patch.path[0]));
    if (myIndex >= 0 &&
...
function createArrayedPath(point, editor) {
    ...
    const childIndex = block.children.findIndex((child) => lodash.isEqual([{ _key: child._key }], childPath));
        ...
        editor.pteToggleBlockStyle = (blockStyle) => {
            ...
            selectedBlocks.forEach(([node, path]) => {
                if (editor.isTextBlock(node) && node.style === blockStyle) {
                    ...
                    slate.Transforms.setNodes(editor, { ...node, style: defaultStyle }, {
                        at: path,
                    });
                }
                else {
    ...
    return function withPortableTextLists(editor) {
        ...
        editor.pteEndList = () => {
            ...
            const selectedBlocks = [
                ...slate.Editor.nodes(editor, {
                    at: editor.selection,
                    match: (node) => slate.Element.isElement(node) &&
...
function createWithPortableTextMarkModel(portableTextFeatures, keyGenerator) {
    return function withPortableTextMarkModel(editor) {
            ...
            if (isSpan || isTextBlock) {
                ...
                for (const op of editor.operations) {
                    ...
                    if (op.type === 'split_node' &&
                        ...
                        if (editor.isTextBlock(targetBlock)) {
                            const oldDefs = (Array.isArray(targetBlock.markDefs) && targetBlock.markDefs) || [];
                            slate.Transforms.setNodes(editor, { markDefs: lodash.uniq([...oldDefs, ...op.properties.markDefs]) }, { at: targetPath, voids: false });
                            editor.onChange();
                        }
                    }
        ...
        editor.apply = (op) => {
            ...
            if (op.type === 'insert_text') {
                ...
                if (selection &&
                    ...
                    ((_b = (_a = slate.Editor.marks(editor)) === null || _a === void 0 ? void 0 : _a.marks) === null || _b === void 0 ? void 0 : _b.some((mark) => !decorators.includes(mark)))) {
                ...
                if (slate.Range.isExpanded(editor.selection)) {
                    ...
                    slate.Editor.withoutNormalizing(editor, () => {
                        splitTextNodes.forEach(([node, path]) => {
                            ...
                            slate.Transforms.setNodes(editor, { marks }, { at: path, match: slate.Text.isText, split: true, hanging: true });
                        });
                    });
        ...
        editor.pteIsMarkActive = (mark) => {
            ...
            if (slate.Range.isExpanded(editor.selection)) {
                Array.from(slate.Editor.nodes(editor, { match: slate.Text.isText, at: editor.selection })).forEach((n) => {
                    const [node] = n;
    ...
    function mergeSpans(editor) {
        ...
        if (selection) {
            for (const [node, path] of Array.from(slate.Editor.nodes(editor, {
                ...
                const [parent] = path.length > 1 ? slate.Editor.node(editor, slate.Path.parent(path)) : [undefined];
    ...
    return function withHotKeys(editor) {
        editor.pteWithHotKeys = (event) => {
            ...
            if (isBackspace &&
                ...
                const [focusBlock] = slate.Editor.node(editor, editor.selection.focus, { depth: 1 });
                if (prevBlock &&
                    focusBlock &&
                    ...
                    slate.Transforms.removeNodes(editor, { match: (n) => n === focusBlock });
            ...
            if (isDelete &&
                ...
                const [focusBlock, focusBlockPath] = slate.Editor.node(editor, editor.selection.focus, { depth: 1 });
                if (nextBlock &&
                    focusBlock &&
                    !slate.Editor.isVoid(editor, focusBlock) &&
            ...
            if (isEnter && !isShiftEnter && editor.selection) {
                ...
                try {
                    ;
                    [focusBlock] = slate.Editor.node(editor, editor.selection.focus, { depth: 1 });
                }
                catch (err) {
                    // Just ignore
                }
    ...
    return function withInsertData(editor) {
            ...
            return blocks
                ...
                if (block._type === blockTypeName) {
                    return block.children
                        ...
                        return `[${((_a = portableTextFeatures.types.inlineObjects.find((t) => t.name === child._type)) === null || _a === void 0 ? void 0 : _a.title) || 'Object'}]`;
                    })
                        .join('');
                }
                return `[${((_a = portableTextFeatures.types.blockObjects.find((t) => t.name === block._type)) === null || _a === void 0 ? void 0 : _a.title) ||
                    'Object'}]`;
            })
                .join('\n\n');
        };
        ...
        editor.insertPortableTextData = (data) => {
            ...
            if (pText) {
                ...
                if (Array.isArray(parsed) && parsed.length > 0) {
                    ...
                    if (!validation.valid) {
                        const errorDescription = `${(_a = validation.resolution) === null || _a === void 0 ? void 0 : _a.description}`;
                        change$.next({
                            type: 'error',
                            level: 'warning',
                            name: 'pasteError',
        ...
        editor.insertTextOrHTMLData = (data) => {
            ...
            if (html || text) {
                ...
                let insertedType;
                if (html) {
                    portableText = blockTools.htmlToBlocks(html, portableTextFeatures.types.portableText).map((block) => blockTools.normalizeBlock(block, { blockTypeName }));
                    ...
                    portableText = blockTools.htmlToBlocks(textToHtml, portableTextFeatures.types.portableText).map((block) => blockTools.normalizeBlock(block, { blockTypeName }));
        ...
        if (editor.isTextBlock(newNode)) {
            newNode.markDefs = newNode.markDefs.map((def) => {
                ...
                if (Array.isArray(newNode.children)) {
                    newNode.children = newNode.children.map((child) => child._type === spanTypeName && slate.Text.isText(child)
                        ? {
                            ...child,
...
class PortableTextEditor extends React__default["default"].Component {
    constructor(props) {
        ...
        this.slateInstance = withPlugins(slate.createEditor(), {
            change$: this.change$,
...
function bufferUntil(emitWhen) {
    return (source) => rxjs.defer(() => {
        ...
        return source.pipe(operators.tap((v) => buffer.push(v)), // add values to buffer
        operators.switchMap(() => (emitWhen(buffer) ? rxjs.of(buffer) : rxjs.EMPTY)), // emit the buffer when the condition is met
        operators.tap(() => (buffer = [])) // clear the buffer
        );
    });
}
...
function normalizeSelection(selection, value) {
    ...
    if (anchor && value.find((blk) => lodash.isEqual({ _key: blk._key }, anchor.path[0]))) {
        newAnchor = normalizePoint(anchor, value);
    }
    if (focus && value.find((blk) => lodash.isEqual({ _key: blk._key }, focus.path[0]))) {
...
const Leaf = (props) => {
    ...
    if (slate.Text.isText(leaf) && leaf._type === portableTextFeatures.types.span.name) {
        ...
        marks.forEach((mark) => {
            ...
            if (type) {
                ...
                if (props.renderDecorator) {
                    returnedChildren = props.renderDecorator(mark, type, { focused, selected, path }, () => React__default["default"].createElement(React__default["default"].Fragment, null, returnedChildren), spanRef);
                }
            }
        });
        ...
        if (annotations.length > 0) {
            annotations.forEach((annotation) => {
                ...
                CustomComponent ? (React__default["default"].createElement(CustomComponent, { ...annotation, attributes: attributes }, returnedChildren)) : (React__default["default"].createElement(React__default["default"].Fragment, null, returnedChildren));
                if (type) {
                    if (props.renderAnnotation) {
                        returnedChildren = (React__default["default"].createElement("span", { ref: spanRef, key: keyGenerator() }, props.renderAnnotation(annotation, type, { focused, selected, path, annotations }, defaultRender, spanRef)));
                    }
                    else {
    ...
    return (React__default["default"].createElement("span", { key: key, ...attributes, ref: spanRef },
        React__default["default"].createElement(DraggableChild, { element: leaf, readOnly: readOnly }, returnedChildren)));
};

const DefaultBlockObject = styled__default["default"].div `
  user-select: none;
  border: ${(props) => {
    if (props.selected) {
...
`;
const DefaultListItem = styled__default["default"].div `
  &.pt-list-item {
    width: fit-content;
    position: relative;
    display: block;
...
`;
const DefaultListItemInner = styled__default["default"].div ``;
...
const DraggableBlock = ({ children, element, readOnly, blockRef }) => {
    ...
    const isDraggingOverBottom = isDraggingOverLastBlock ||
        (isDragOver &&
            ...
            dragPosition === 'bottom');
    const dropIndicator = React.useMemo(() => (React__default["default"].createElement("div", { className: "pt-drop-indicator", style: {
            position: 'absolute',
            width: '100%',
            height: 1,
...
const Element = ({ attributes, children, element, portableTextFeatures, readOnly, renderBlock, renderChild, spellCheck, }) => {
    ...
    if (editor.isInline(element)) {
        ...
        if (slate.Element.isElement(block)) {
            ...
            return (React__default["default"].createElement("span", { ...attributes },
                children,
    ...
    if (element._type === portableTextFeatures.types.block.name) {
        ...
        const textBlock = (React__default["default"].createElement(TextBlock, { block: element, portableTextFeatures: portableTextFeatures }, children));
        const propsOrDefaultRendered = renderBlock
            ...
            : textBlock;
        return (React__default["default"].createElement("div", { key: element._key, ...attributes, className: className, spellCheck: spellCheck },
    ...
    return (React__default["default"].createElement("div", { key: element._key, ...attributes, className: className },
        children,
        React__default["default"].createElement(DraggableBlock, { element: element, readOnly: readOnly, blockRef: blockRef },
...
const PortableTextEditable = React.forwardRef(function PortableTextEditable(props, forwardedRef) {
    ...
    const isEmpty = React.useMemo(() => isEqualToEmptyEditor(slateEditor.children, portableTextFeatures), [portableTextFeatures, slateEditor.children]);
    ...
    const withEditableAPI = React.useMemo(() => createWithEditableAPI(portableTextEditor, portableTextFeatures, keyGenerator), [keyGenerator, portableTextEditor, portableTextFeatures]);
    // Update the Slate instance's plugins which are dependent on props for Editable
    React.useMemo(() => {
        if (readOnly) {
            debug('Editable is in read only mode');
            return slateReact.withReact(slateEditor);
        }
        debug('Editable is in edit mode');
        return withEditableAPI(withInsertData(withHotKeys(slateReact.withReact(slateEditor))));
    }, [readOnly, slateEditor, withEditableAPI, withHotKeys, withInsertData]);
    const renderElement = React.useCallback((eProps) => (React__default["default"].createElement(Element, { ...eProps, portableTextFeatures: portableTextFeatures, readOnly: readOnly, renderBlock: renderBlock, renderChild: renderChild, spellCheck: spellCheck })), [portableTextFeatures, spellCheck, readOnly, renderBlock, renderChild]);
    const renderLeaf = React.useCallback((lProps) => {
        if (renderPlaceholder && lProps.leaf.placeholder && lProps.text.text === '') {
            return (React__default["default"].createElement(React__default["default"].Fragment, null,
                ...
                React__default["default"].createElement(Leaf, { ...lProps, keyGenerator: keyGenerator, portableTextFeatures: portableTextFeatures, renderAnnotation: renderAnnotation, renderChild: renderChild, renderDecorator: renderDecorator, readOnly: readOnly })));
        }
        return (React__default["default"].createElement(Leaf, { ...lProps, keyGenerator: keyGenerator, portableTextFeatures: portableTextFeatures, renderAnnotation: renderAnnotation, renderChild: renderChild, renderDecorator: renderDecorator, readOnly: readOnly }));
    }, [
        readOnly,
        keyGenerator,
        portableTextFeatures,
    ...
    // The editor
    const slateEditable = React.useMemo(() => (React__default["default"].createElement(slateReact.Editable, { autoFocus: false, className: "pt-editable", decorate: decorate, onBlur: handleOnBlur, onCopy: handleCopy, onDOMBeforeInput: handleOnBeforeInput, onFocus: handleOnFocus, onKeyDown: handleKeyDown, onPaste: handlePaste, readOnly: readOnly, renderElement: renderElement, renderLeaf: renderLeaf, scrollSelectionIntoView: scrollSelectionIntoViewToSlate })), [
        decorate,
        handleCopy,
        handleKeyDown,
        handleOnBeforeInput,
    ...
    return (React__default["default"].createElement("div", { ref: ref, ...restProps }, slateEditable));
});
