...
jest.mock('./util/convertToValidationMarker', () => {
    return {
        convertToValidationMarker: jest.fn(jest.requireActual('./util/convertToValidationMarker').convertToValidationMarker),
    };
});
beforeEach(() => {
    ;
...
describe('validateItem', () => {
    ...
    it.skip('runs nested validation for markDefs', async () => {
        const linkValidationSpy = jest.fn(() => true);
                                        ...
                                        annotations: [
                                            {
                                                ...
                                                validation: (rule) => rule.custom(linkValidationSpy),
                                            },
                                            {
                                                name: 'internalLink',
                                                type: 'object',
                                                ...
                                                validation: (rule) => rule.custom(internalLinkSpy),
                                            },
    ...
    it('properly passes the nested value, type, and path to rule.validate', async () => {
            ...
            types: [
                {
                    ...
                    fields: [
                        {
                            name: 'level1Object',
                            type: 'object',
                            validation: (rule) => rule.custom(() => 'from level 1 object'),
                            fields: [
                                {
                                    name: 'level2String',
                                    type: 'string',
                                    validation: (rule) => rule.custom(() => 'from level 2 via object'),
                                },
                            ],
                        },
                        {
                            name: 'level1Array',
                            type: 'array',
                            validation: (rule) => rule.custom(() => 'from level 1 array'),
                            of: [
                                {
                                    type: 'object',
                                    fields: [
                                        {
                                            name: 'level2Number',
                                            type: 'number',
                                            validation: (rule) => rule.custom(() => 'from level 2 via array'),
                                        },
                                    ],
                                },
                            ],
                    ...
                    validation: (rule) => rule.custom(() => 'from root'),
                },
            ],
        });
        // ensures there are no schema formatting issues
        ...
        const getField = ({ in: type, name }) => {
            const result = 'fields' in type && type.fields.find((f) => f.name === name)?.type;
            if (!result)
                throw new Error(`Could not find field \`${name}\` in \`${type.name}\``);
            return result;
        };
    ...
    it('runs all nested validation checks concurrently', async () => {
        ...
        const customValidationFn = () => new Promise((resolve) => resolvers.add(resolve));
        const schema = createSchema({
            types: [
                {
                    name: 'root',
                            ...
                            fields: [
                                {
                                    name: 'level2String',
                                    type: 'string',
                                    validation: (rule) => rule.custom(customValidationFn),
                                },
                            ],
                        },
                        {
...
//# sourceMappingURL=validateDocument.test.js.map