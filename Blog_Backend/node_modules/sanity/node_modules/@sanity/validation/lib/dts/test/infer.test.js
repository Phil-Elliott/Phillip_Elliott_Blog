...
describe('schema validation inference', () => {
    describe('object with `options.list` and `value` field', () => {
        ...
        test('disallowed value', async () => {
            const type = inferFromSchema(schema).get('colorList');
            await expectError(type.validation, { value: '#ccc', title: 'Gray' }, 'Value did not match any allowed value');
        });
    });
    describe('field validations', () => {
        ...
        test('field validations defined on an object type does not affect the field type validation', () => {
            ...
            const fieldWithoutValidation = documentType.fields.find((field) => field.name === 'stringField');
            // The first field should only have the validation rules that comes with its type
            expect((fieldWithoutValidation?.type.validation).flatMap(
            // eslint-disable-next-line dot-notation
            (validation) => validation['_rules'])).toEqual([{ flag: 'type', constraint: 'String' }]);
        });
    });
    describe('slug validation', () => {
        ...
        test('slug is valid if uniqueness queries returns true', async () => {
            client.fetch.mockImplementation(() => Promise.resolve(
    ...
    describe('reference validation', () => {
        ...
        test('referenced document must exist (unless weak)', async () => {
            ...
            await expect(validateDocument(getClient, {
                ...mockDocument,
                referenceField: {
                    _ref: 'example-id',
                },
            }, schema, { getDocumentExists: mockGetDocumentExists })).resolves.toMatchObject([
                {
                    item: { message: /.+/ },
                    level: 'error',
                    path: ['referenceField'],
        ...
        test('throws if `getDocumentExists` is not present', async () => {
            const result = await validateDocument(getClient, {
                ...mockDocument,
                referenceField: {
                    _ref: 'example-id',
                },
            }, schema, { getDocumentExists: undefined });
            expect(result).toHaveLength(1);
            expect(result[0].item.message.includes('`getDocumentExists` was not provided in validation context')).toBe(true);
        });
        test('reference is valid if schema type is strong and document does exists', async () => {
            ...
            }, schema, { getDocumentExists: mockGetDocumentExists })).resolves.toEqual([]);
            expect(mockGetDocumentExists.mock.calls).toMatchObject([[{ id: 'example-id' }]]);
        });
    });
});
async function expectNoError(validations, value) {
    const errors = (await Promise.all(validations.map((rule) => rule.validate(value, { client: client, getClient, schema: {} })))).flat();
    if (errors.length === 0) {
...
//# sourceMappingURL=infer.test.js.map