...
function createFieldsGroups(typeDef, fields) {
    ...
    if (defaultGroups.length > 1) {
        ...
        throw new Error(`You currently have ${defaultGroups.length} default field groups defined for type '${typeDef.name ? startCase(typeDef.name) : typeDef.title ?? ``}', but only 1 is supported`);
    }
...
function buildTitle$1(type) {
    ...
    return `Reference to ${humanize$1(arrify(type.to).map((toType) => toType.title), 'or').toLowerCase()}`;
}
const ReferenceType = {
    get() {
...
function buildTitle(type) {
    ...
    return `Cross dataset reference to ${humanize(arrify(type.to).map((toType) => toType.title || capitalize(toType.type)), 'or').toLowerCase()}`;
}
