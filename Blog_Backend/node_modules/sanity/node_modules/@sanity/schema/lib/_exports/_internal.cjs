...
function createTypeWithMembersProblemsAccessor(memberPropertyName, getMembers = (type) => lodash.get(type, memberPropertyName)) {
...
function validateInputComponent(typeDef) {
    if ('components' in typeDef && !isInputComponentLike(typeDef.components.input)) {
        return [
            warning(`The \`components.input\` property is set but does not appear to be a valid React component (expected a function, but saw ${inspect__default["default"](typeDef.components.input)}). If you have imported a custom input component, please verify that you have imported the correct named/default export.`),
        ];
    }
    return [];
}
...
function validateFieldName(name) {
    if (typeof name !== 'string') {
        return [
            error(`Field names must be strings. Saw "${inspect__default["default"](name)}"`, HELP_IDS.OBJECT_FIELD_NAME_INVALID),
        ];
    }
...
function validateField(field, _visitorContext) {
    if (!lodash.isPlainObject(field)) {
        return [
            error(`Incorrect type for field definition - should be an object, saw ${inspect__default["default"](field)}`, HELP_IDS.OBJECT_FIELD_DEFINITION_INVALID_TYPE),
        ];
    }
    const problems = [];
...
function validateBlockType(typeDef, visitorContext) {
    ...
    if (disallowedKeys.length > 0) {
        problems.push(error(`Found unknown properties for block declaration: ${humanizeList__default["default"](disallowedKeys.map(quote$1))}`));
    }
    if (marks) {
    ...
    return {
        ...lodash.omit(typeDef, disallowedKeys),
        marks,
        styles,
        of: members,
        _problems: problems,
...
function validateMarks(marks, visitorContext, problems) {
    ...
    if (disallowedMarkKeys.length > 0) {
        problems.push(error(`Found unknown properties for block declaration: ${humanizeList__default["default"](disallowedMarkKeys.map(quote$1))}`));
    }
...
function validateStyles(styles, visitorContext, problems) {
    ...
    styles.forEach((style, index) => {
        ...
        if (disallowedKeys.length > 0) {
            problems.push(error(`Found unknown properties for style ${name}: ${humanizeList__default["default"](disallowedKeys.map(quote$1))}`));
        }
        if (!style.value) {
...
function validateDecorators(decorators, visitorContext, problems) {
    decorators.forEach((decorator, index) => {
        ...
        if (disallowedKeys.length > 0) {
            problems.push(error(`Found unknown properties for decorator ${name}: ${humanizeList__default["default"](disallowedKeys.map(quote$1))}`));
        }
        if (!decorator.value) {
...
function validateTypeName(typeName, visitorContext) {
    ...
    if (!typeName) {
        return [
            error(`Type is missing a type. Valid types are: ${humanizeList__default["default"](possibleTypeNames)}`, HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
    if (typeof typeName !== 'string') {
        return [
            error(`Type has an invalid "type"-property - should be a string. Valid types are: ${humanizeList__default["default"](possibleTypeNames)}`, HELP_IDS.TYPE_MISSING_TYPE),
        ];
    }
    ...
    if (!isValid) {
        ...
        const suggestion = suggestions.length > 0
            ? ` Did you mean ${humanizeList__default["default"](suggestions.map(quote), { conjunction: 'or' })}?`
            : '';
        return [
            error(`Unknown type: ${typeName}.${suggestion}\n\nValid types are: ${humanizeList__default["default"](possibleTypeNames)}`),
        ];
    }
    return [];
}
