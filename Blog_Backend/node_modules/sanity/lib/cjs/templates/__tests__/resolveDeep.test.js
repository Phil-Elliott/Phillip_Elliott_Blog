...
function generateNestedObjectTest(obj, maxDepth, depth) {
  depth = depth || 1;
...
describe('resolveDeepInitialValues', () => {
  test('resolves deep, recursive default values', async () => {
    const defaultTemplates = (0, _prepare.defaultTemplatesForSchema)(_schema.schema);
    ...
    const initialValue = await (0, _.resolveInitialValue)(_schema.schema, developerTemplate, {}, mockConfigContext);
  ...
  test('resolves deep primitive type with initial value in field', async () => {
    ...
    const initialValue = await (0, _.resolveInitialValue)(_schema.schema, personTemplate, undefined, mockConfigContext);
    expect(initialValue).toEqual({
      _type: 'person',
      address: {
        _type: 'address',
  ...
  test('resolves deep primitive type and overrides child initial value in field by parents', async () => {
    ...
    const initialValue = await (0, _.resolveInitialValue)(_schema.schema, { ...personTemplate,
      value: { ...(0, _resolve.resolveValue)(personTemplate.value, undefined, mockConfigContext),
        address: {
          _type: 'address',
