...
function getTemplatePermissions(_ref) {
  ...
  return (0, _rxjs.combineLatest)(templateItems.map(serialize).map(async item => {
    ...
    const resolvedInitialValue = await (0, _templates.resolveInitialValue)(schema, template, item.parameters, context);
    return {
      template,
      item,
...
function useTemplatePermissions(_ref4) {
  ...
  const initialValueContext = (0, _document.useInitialValueResolverContext)();
