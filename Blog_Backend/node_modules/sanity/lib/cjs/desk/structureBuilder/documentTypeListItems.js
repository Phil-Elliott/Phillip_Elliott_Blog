...
function getDocumentTypeListItem(context, typeName) {
  ...
  return new _ListItem.ListItemBuilder(context).id(typeName).title(title).schemaType(type).child((id, childContext) => {
...
function getDocumentTypeList(context, typeNameOrSpec) {
  ...
  new _MenuItem.MenuItemBuilder(context).group('layout').title('Compact view').icon(_icons.StackCompactIcon).action('setLayout').params({
    layout: 'default'
  }), new _MenuItem.MenuItemBuilder(context).group('layout').title('Detailed view').icon(_icons.StackIcon).action('setLayout').params({
    layout: 'detail'
