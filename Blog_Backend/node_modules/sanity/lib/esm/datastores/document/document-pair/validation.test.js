...
import { validation } from './validation'; // Mock `./editState`
...
function createSubscription(client, documentPreviewStore) {
  ...
  }, 'movie').pipe(publish()) // Publish and connect this for the tests
  ;
  stream.connect(); // Create a subject we can use to notify via `done.next()`

  const done = new Subject(); // Create a subscription that collects all emissions until `done.next()`
...
describe('validation', () => {
  ...
  it.skip('re-runs validation when dependency events change', async () => {
    ...
    mockPreviewStore.unstable_observeDocumentPairAvailability = () => concat(of({
      published: {
        available: true
      }
    }), subject.pipe(mapTo({
    ...
    mockPreviewStore.unstable_observeDocumentPairAvailability = id => id === 'example-ref-id' ? of({
      published: {
        available: false
      }
    }) : of({
  ...
  it.skip('replays the last known version via `memoize` and `publishReplay`', async () => {
    const client = createMockSanityClient(); // Mock `editState`
    ...
    const subscription = validation({
      ...
      publishedId: 'example-id',
      draftId: 'drafts.example-id'
    }, 'movie').pipe(buffer(timer(500))).toPromise(); // simulate first emission from validation listener
