...
import { distinctUntilChanged, filter, map, mergeMap, mergeMapTo, publishReplay, refCount, scan, share, take, tap, withLatestFrom } from 'rxjs/operators';
...
export const createObservableBufferedDocument = (listenerEvent$, commitMutations) => {
  // Incoming local actions (e.g. a request to mutate, a request to commit pending changes, etc.)
  const actions$ = new Subject(); // Stream of commit requests. Must be handled by a commit handler

  const consistency$ = new BehaviorSubject(true); // Stream of mutations for this document
  // NOTE: this will *not* include remote mutations received over the listener
  // that has *already* applied locally/optimistically

  const mutations$ = new Subject(); // a stream of rebase events emitted from the mutator

  const rebase$ = new Subject(); // a stream of remote mutations with effetcs
  ...
  const createInitialBufferedDocument = initialSnapshot => {
    ...
    bufferedDocument.commitHandler = opts => {
      ...
      commitMutations(mutation).then(opts.success, error => {
        const isBadRequest = 'statusCode' in error && typeof error.statusCode === 'number' && error.statusCode >= 400 && error.statusCode <= 500;
  ...
  const currentBufferedDocument$ = listenerEvent$.pipe(scan((bufferedDocument, listenerEvent) => {
    ...
    return bufferedDocument;
  }, null), distinctUntilChanged(), publishReplay(1), refCount()); // this is a stream of document snapshots where each new snapshot are emitted after listener mutations
  ...
  const snapshotAfterSync$ = listenerEvent$.pipe(filter(ev => ev.type === 'mutation'), withLatestFrom(currentBufferedDocument$), map(_ref2 => {
  ...
  const actionHandler$ = actions$.pipe(withLatestFrom(currentBufferedDocument$), tap(_ref3 => {
    let [action, bufferedDocument] = _ref3;

    if (action.type === 'mutation') {
      bufferedDocument.add(new Mutation({
  ...
  const commit = () => {
    return currentBufferedDocument$.pipe(take(1), mergeMap(bufferedDocument => bufferedDocument.commit()), mergeMapTo(EMPTY));
