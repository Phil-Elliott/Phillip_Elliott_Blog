...
export async function resolveInitialValue(schema, template) {
  ...
  resolvedValue = validateInitialObjectValue(resolvedValue, template); // Get deep initial values from schema types (note: the initial value from template overrides the types)
  ...
  const newValue = deepAssign((await resolveInitialValueForType(schemaType, params, DEFAULT_MAX_RECURSION_DEPTH, context)) || {}, resolvedValue); // revalidate and return new initial values
...
async function resolveInitialArrayValue(type, params, maxDepth, context) {
  ...
  return Promise.all(initialArray.map(async initialItem => {
    ...
    return isObjectSchemaType(itemType) ? { ...initialItem,
