...
import { DateInput } from '../DateInput'; // NOTE: for the tests to be deterministic we need this to ensure tests are run in a predefined timezone
...
test('does not emit onChange after invalid value has been typed', async () => {
  const {
    ...
    render: inputProps => /*#__PURE__*/React.createElement(DateInput, inputProps)
  });
...
test('emits onChange on correct format if a valid value has been typed', async () => {
  ...
  fireEvent.blur(input); // NOTE: the date is entered and displayed in local timezone but stored in utc
...
test('formatting of deserialized value', async () => {
  ...
  const input = result.container.querySelector('input'); // const {textInput} = renderInput({value: '2021-03-28'} as any)
