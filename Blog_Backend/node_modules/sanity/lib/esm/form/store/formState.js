...
import { isArrayOfObjectsSchemaType, isArraySchemaType, isObjectSchemaType } from '@sanity/types';
...
groupsConfig, field, currentGroup) {
  ...
  return _castArray(field.group).includes(currentGroup.name);
}
...
function everyItemHasKey(array) {
  return array === null || array === void 0 ? void 0 : array.every(item => isRecord(item) && hasKey(item));
}
...
function isChangedValue(value, comparisonValue) {
  ...
  return !_isEqual2(value, comparisonValue);
}
/*
...
function prepareFieldMember(props) {
  ...
  if (isObjectSchemaType(field.type)) {
    ...
    const fieldComparisonValue = isRecord(parentComparisonValue) ? parentComparisonValue === null || parentComparisonValue === void 0 ? void 0 : parentComparisonValue[field.name] : undefined;
...
function prepareObjectInputState(props) {
  ...
  const hidden = enableHiddenCheck && resolveConditionalProperty(props.schemaType.hidden, conditionalPropertyContext);
  ...
  const members = normalizedSchemaMembers.flatMap((fieldSet, index) => {
    ...
    const fieldsetHidden = resolveConditionalProperty(fieldSet.hidden, {
      ...
      document: props.document,
      parent: props.value,
      value: _pick(props.value, fieldsetFieldNames)
    });
    ...
    const fieldsetMembers = fieldSet.fields.flatMap(field => {
      ...
      const fieldStateWithReadOnly = { ...fieldState,
        field: { ...(fieldState === null || fieldState === void 0 ? void 0 : fieldState.field),
  ...
  const validation = props.validation.filter(item => isEqual(item.path, props.path)).map(v => ({
    level: v.level,
...
function prepareArrayOfPrimitivesInputState(props) {
  ...
  const readOnly = props.readOnly || resolveConditionalProperty(props.schemaType.readOnly, conditionalPropertyContext); // Todo: improve error handling at the parent level so that the value here is either undefined or an array
...
export function prepareFormState(props) {
