...
export function ArrayOfObjectsField(props) {
  ...
  const handleInsert = useCallback(event => {
    onChange(PatchEvent.from([setIfMissing([]), insert(event.items.map(item => ensureKey(item)), event.position, [event.referenceItem])]).prefixAll(member.name));
  ...
  const handlePrependItem = useCallback(item => {
    onChange(PatchEvent.from([setIfMissing([]), insert([ensureKey(item)], 'before', [0])]).prefixAll(member.name));
  }, [member.name, onChange]);
  const handleAppendItem = useCallback(item => {
    onChange(PatchEvent.from([setIfMissing([]), insert([ensureKey(item)], 'after', [-1])]).prefixAll(member.name));
  }, [member.name, onChange]);
  ...
  const handleCollapseItem = useCallback(itemKey => {
    onSetPathCollapsed(member.field.path.concat({
      _key: itemKey
    }), true);
  }, [onSetPathCollapsed, member.field.path]);
  const handleExpandItem = useCallback(itemKey => {
    onSetPathCollapsed(member.field.path.concat({
      _key: itemKey
    }), false);
  }, [onSetPathCollapsed, member.field.path]);
  const handleOpenItem = useCallback(path => {
    onPathOpen(path);
  ...
  const handleRemoveItem = useCallback(itemKey => {
    onChange(PatchEvent.from([unset(member.field.path.concat({
      _key: itemKey
    }))]));
  ...
  }, [member.field.level, member.field.members, member.field.value, member.field.readOnly, member.field.schemaType, member.field.changed, member.field.id, member.field.focusPath, member.field.focused, member.field.path, member.field.validation, member.field.presence, handleExpand, handleCollapse, handleExpandItem, handleCollapseItem, handleCloseItem, handleOpenItem, handleChange, handleInsert, handleMoveItem, handleRemoveItem, handleAppendItem, handlePrependItem, handleFocusChildPath, renderInput, renderField, renderItem, renderPreview, elementProps]);
  ...
  return /*#__PURE__*/React.createElement(FormCallbacksProvider, {
