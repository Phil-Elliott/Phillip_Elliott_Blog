 ...
 * @alpha
 */
export function ArrayOfObjectsItem(props) {
  ...
  const onInsert = useCallback(event => {
    onChange(PatchEvent.from([insert(event.items.map(item => ensureKey(item)), event.position, [{
      _key: member.key
    }])]));
  }, [member.key, onChange]);
  ...
  const handleChange = useCallback(event => {
    onChange(PatchEvent.from(event).prepend(setIfMissing(createProtoValue(member.item.schemaType))).prefixAll({
      _key: member.key
    }));
