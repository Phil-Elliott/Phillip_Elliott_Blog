...
const reportErrors = _debounce(() => {
  /* eslint-disable no-console */
  const errorsByType = errorCollector.getAll();

  const uniqueErrors = _flatten(Object.keys(errorsByType).map(typeName => {
    const entries = errorsByType[typeName];
    return _uniqBy(entries, entry => entry.error.message);
  }));
  ...
  Object.keys(errorsByType).forEach(typeName => {
    ...
    const uniqued = _uniqBy(entries, entry => entry.error.message);
...
function validatePreparedValue(preparedValue) {
  if (!_isPlainObject(preparedValue) || preparedValue === null) {
...
export function invokePrepare(type, value) {
  var _type$preview;
...
function getListOptions(type) {
  ...
  return listOptions.map(option => isTitledListValue(option) ? option : {
    title: option,
    value: option
  });
}

export function prepareForPreview(rawValue, type) {
  ...
  const selectedValue = targetKeys.reduce((acc, key) => {
    ...
    if (!hasCustomPrepare && listOptions) {
      ...
      const selectedOption = listOptions && listOptions.find(opt => opt.value === _get(rawValue, selection[key]));
      acc[key] = selectedOption ? selectedOption.value : _get(rawValue, selection[key]);
    } else {
      acc[key] = _get(rawValue, selection[key]);
    }
  ...
  return returnValueResult.errors.length > 0 ? withErrors(returnValueResult, type, selectedValue) : { ..._pick(rawValue, PRESERVE_KEYS),
