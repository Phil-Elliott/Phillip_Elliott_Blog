...
function getDocumentKey(value, index) {
  return value._id ? prepareConfig.getPublishedId(value._id) : "item-".concat(index);
}

function removePublishedWithDrafts(documents) {
  return prepareConfig.collate(documents).map(entry => {
...
function DocumentListPaneContent(props) {
  ...
  const renderItem = React.useCallback(item => {
    const publishedId = prepareConfig.getPublishedId(item._id);
  ...
  const content = React.useMemo(() => {
    ...
    return /* @__PURE__ */jsxRuntime.jsxs(ui.Box, {
      ...
      }, "".concat(index, "-").concat(collapsed)), isLoading && /* @__PURE__ */jsxRuntime.jsx(ui.Card, {
        borderTop: true,
        marginTop: 1,
        paddingX: 3,
        paddingY: 4,
  ...
  return /* @__PURE__ */jsxRuntime.jsx(prepareConfig.PaneContent, {
...
const getQueryResults = function (receivedProps$) {
  ...
  const queryResults$ = queryProps$.pipe(operators.switchMap(queryProps => {
    const query$ = prepareConfig.listenQuery(queryProps.client, queryProps.query, queryProps.params, options).pipe(operators.map(createResultChildProps), operators.share());
...
function useDocumentList(opts) {
  ...
  const client = prepareConfig.useClient(prepareConfig.DEFAULT_STUDIO_CLIENT_OPTIONS);
...
const DocumentListPane = React.memo(function DocumentListPane2(props) {
  ...
  const [layout, setLayout] = prepareConfig.useDeskToolSetting(typeName, "layout", defaultLayout);
  const [sortOrderRaw, setSortOrder] = prepareConfig.useDeskToolSetting(typeName, "sortOrder", DEFAULT_ORDERING);
  const sortWithOrderingFn = typeName && sortOrderRaw ? applyOrderingFunctions(sortOrderRaw, schema.get(typeName)) : sortOrderRaw;
  const sortOrder = prepareConfig.useUnique(sortWithOrderingFn);
  ...
  return /* @__PURE__ */jsxRuntime.jsx(prepareConfig.SourceProvider, {
    name: sourceName || parentSourceName,
    children: /* @__PURE__ */jsxRuntime.jsxs(prepareConfig.Pane, {
      currentMaxWidth: 350,
      id: paneKey,
      maxWidth: 640,
      minWidth: 320,
      selected: isSelected,
      children: [prepareConfig._DEBUG, /* @__PURE__ */jsxRuntime.jsx(DocumentListPaneHeader, {
        index,
        initialValueTemplates,
        menuItems,
        menuItemGroups,
...
//# sourceMappingURL=_index-6c9b395a.cjs.map
