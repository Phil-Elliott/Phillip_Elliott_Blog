...
var _internal = require('./__internal-e18d47d0.cjs');
...
async function getStudioConfig(options) {
  ...
  try {
    ...
    workspaces = await resolveConfig.resolveConfig(config).pipe(operators.first()).toPromise();
  } catch (error) {
    if (cleanup) {
      cleanup();
    }
...
async function getGraphQLAPIs(cliContext) {
  ...
  const defaultSchema = prepareConfig.createSchema({
    name: "default",
    types: []
  });
  ...
  const resolved = apis.map(_ref => {
    ...
    return _objectSpread({
      schema: prepareConfig.createSchema({
        name: "default",
...
//# sourceMappingURL=_deployApiAction-c1bb6197.cjs.map
