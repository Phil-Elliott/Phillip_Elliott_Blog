...
function prepareFieldMember(props) {
    ...
    if (isObjectSchemaType(field.type)) {
        ...
        if (isArrayOfObjectsSchemaType(field.type)) {
            ...
            const scopedCollapsedPaths = parent.collapsedPaths?.children?.[field.name];
            const scopedCollapsedFieldSets = parent.collapsedFieldSets?.children?.[field.name];
            const readOnly = parent.readOnly ||
...
function prepareObjectInputState(props, enableHiddenCheck = true) {
    ...
    const hidden = enableHiddenCheck &&
    ...
    const normalizedSchemaMembers = props.schemaType.fieldsets
    ...
    const members = normalizedSchemaMembers.flatMap((fieldSet, index) => {
        ...
        const collapsed = (props.collapsedFieldSets?.children || {})[fieldSet.name]?.value ??
            defaultCollapsedState.collapsed;
        return [
            {
                kind: 'fieldSet',
...
function prepareArrayOfPrimitivesInputState(props) {
    ...
    const validation = props.validation
        .filter((item) => isEqual(item.path, props.path))
        .map((v) => ({ level: v.level, message: v.item.message, path: v.path }));
    const members = items.flatMap((item, index) => prepareArrayOfPrimitivesMember({ arrayItem: item, parent: props, index }));
    return {
...
function prepareArrayOfPrimitivesMember(props) {
    ...
    const itemValue = parent.value?.[index];
    const itemComparisonValue = parent.comparisonValue?.[index];
    const itemLevel = parent.level + 1;
