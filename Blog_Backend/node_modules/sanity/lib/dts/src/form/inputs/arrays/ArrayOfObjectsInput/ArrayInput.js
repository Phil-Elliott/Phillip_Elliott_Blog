...
export class ArrayInput extends React.PureComponent {
    constructor() {
        ...
        this.renderItem = (itemProps) => {
            ...
            return (React.createElement(ArrayItem, { changed: itemProps.changed, validation: itemProps.validation, readOnly: itemProps.readOnly, onInsert: itemProps.onInsert, onRemove: itemProps.onRemove, onFocus: itemProps.onFocus, index: itemProps.index, schemaType: itemProps.schemaType, layout: schemaType.options?.layout, insertableTypes: schemaType.of, value: itemProps.value, focused: focused, open: itemProps.open, path: itemProps.path, onClick: itemProps.onOpen, presence: itemProps.presence, renderPreview: renderPreview }, itemProps.open ? (React.createElement(Dialog, { width: 1, header: `Edit ${typeTitle}`, id: `${id}-item-${itemProps.key}-dialog`, onClose: itemProps.onClose },
    ...
    renderMember(member) {
        ...
        if (member.kind === 'item') {
            return (React.createElement(ArrayOfObjectsItem, { member: member, renderItem: this.renderItem, renderField: renderField, renderInput: renderInput, renderPreview: renderPreview }));
        }
    ...
    render() {
        const { schemaType, onChange, value = [], readOnly, members, elementProps, resolveUploader, } = this.props;
        ...
        return (React.createElement(Stack, { space: 3 },
            React.createElement(UploadTarget, { types: schemaType.of, resolveUploader: resolveUploader, onUpload: this.handleUpload, ...elementProps, tabIndex: 0 },
