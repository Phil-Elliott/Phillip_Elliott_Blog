...
export function ImageToolInput(props) {
    const { imageUrl, value, changed, level, path, focusPath = EMPTY_ARRAY, presence, onChange, schemaType, onFocusPath, readOnly, elementProps, } = props;
    ...
    return (React.createElement(FormField, { title: "Hotspot & crop", level: level, description: "Adjust the rectangle to crop image. Adjust the circle to specify the area that should always be visible.", __unstable_presence: presence },
        React.createElement("div", null,
            React.createElement(Card, { __unstable_checkered: true, __unstable_focusRing: true, tabIndex: 0, ref: elementProps.ref, onFocus: handleFocus },
                ...
                React.createElement(Grid, { columns: PREVIEW_ASPECT_RATIOS.length, gap: 1 }, PREVIEW_ASPECT_RATIOS.map(([title, ratio]) => (React.createElement("div", { key: ratio },
                    ...
                    React.createElement(Box, { marginTop: 2 },
                        React.createElement(RatioBox, { ratio: ratio },
                            React.createElement(Card, { __unstable_checkered: true }, !isImageLoading && image ? (React.createElement(HotspotImage, { aspectRatio: ratio, src: image.src, srcAspectRatio: image.width / image.height, hotspot: localValue.hotspot || DEFAULT_HOTSPOT, crop: localValue.crop || DEFAULT_CROP })) : (React.createElement(Placeholder, null)))))))))))));
}
