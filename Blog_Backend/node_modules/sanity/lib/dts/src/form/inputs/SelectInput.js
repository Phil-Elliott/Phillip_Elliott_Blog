...
export function SelectInput(props) {
    const { value, readOnly, validationError, schemaType, onChange, path, changed, focused, elementProps, } = props;
    ...
    const content = isRadio ? (React.createElement(RadioSelect, { ...elementProps, value: currentItem, inputId: inputId, items: items, direction: schemaType.options?.direction || 'vertical', customValidity: validationError, onChange: handleChange })) : (React.createElement(Select, { ...elementProps, customValidity: validationError, value: optionValueFromItem(currentItem), onChange: handleSelectChange }, [EMPTY_ITEM, ...items].map((item, i) => (React.createElement("option", { key: `${i - 1}`, value: i - 1 }, item.title)))));
