...
export function ArrayOfObjectsField(props) {
    ...
    const handleInsert = useCallback((event) => {
        onChange(PatchEvent.from([
            setIfMissing([]),
            insert(event.items.map((item) => ensureKey(item)), event.position, [event.referenceItem]),
        ]).prefixAll(member.name));
    }, [member.name, onChange]);
    const handleMoveItem = useCallback((event) => {
        const value = member.field.value;
        const item = value?.[event.fromIndex];
        const refItem = value?.[event.toIndex];
    ...
    return (React.createElement(FormCallbacksProvider, { onFieldGroupSelect: onFieldGroupSelect, onChange: handleChange, onSetFieldSetCollapsed: onSetFieldSetCollapsed, onSetPathCollapsed: onSetPathCollapsed, onPathOpen: onPathOpen, onPathBlur: onPathBlur, onPathFocus: onPathFocus }, useMemo(() => renderField(fieldProps), [fieldProps, renderField])));
}
//# sourceMappingURL=ArrayOfObjectsField.js.map