...
const reportErrors = debounce(() => {
    ...
    const uniqueErrors = flatten(Object.keys(errorsByType).map((typeName) => {
    ...
    console.groupCollapsed(`%cHeads up! Got ${errorCount === 1 ? 'error' : `${errorCount} errors`} while preparing data for preview. Click for details.`, 'color: #ff7e7c');
    Object.keys(errorsByType).forEach((typeName) => {
        ...
        uniqued.forEach((entry) => {
            if (entry.error.type === 'returnValueError') {
                ...
                console.log(`Encountered an invalid ${hasPrepare
                    ? 'return value when calling prepare(%o)'
                    : 'value targeted by preview.select'}:`, value);
                console.error(error);
            }
...
const isRenderable = (fieldName) => (value) => {
    ...
    return [
        assignType('returnValueError', new Error(`The "${fieldName}" field should be a string, number, boolean, undefined or null, instead saw ${inspect(value)}`)),
    ];
};
...
function validatePreparedValue(preparedValue) {
    if (!isPlainObject(preparedValue) || preparedValue === null) {
        return [
            assignType('returnValueError', new Error(`Invalid return value. Expected a plain object with at least a 'title' field, instead saw ${inspect(preparedValue)}`)),
        ];
    }
    return Object.entries(preparedValue).reduce((acc, [fieldName, fieldValue]) => {
...
export function invokePrepare(type, value, viewOptions = {}) {
...
export function prepareForPreview(rawValue, type, viewOptions = {}) {
    ...
    const selectedValue = targetKeys.reduce((acc, key) => {
        ...
        if (!hasCustomPrepare && listOptions) {
            ...
            const selectedOption = listOptions && listOptions.find((opt) => opt.value === get(rawValue, selection[key]));
...
//# sourceMappingURL=prepareForPreview.js.map