...
function FallbackIcon() {
    return React.createElement(DocumentIcon, { className: "sanity-studio__preview-fallback-icon" });
}
export function SanityDefaultPreview(props) {
    const { description: descriptionProp, icon, layout, media: mediaProp, subtitle: subtitleProp, title: titleProp, value: valueProp, ...restProps } = props;
    const client = useClient(DEFAULT_STUDIO_CLIENT_OPTIONS);
    const imageBuilder = useMemo(() => imageUrlBuilder(client), [client]);
    const component = (_previewComponents[layout || 'default'] ||
        _previewComponents.default);
    ...
    const title = titleProp || (isRecord(value) && value?.title ? String(value?.title) : undefined);
    const subtitle = subtitleProp || (isRecord(value) && value?.subtitle ? String(value?.subtitle) : undefined);
    ...
    const renderMedia = useCallback((options) => {
        ...
        // Handle sanity image
        return (React.createElement("img", { alt: isString(title) ? title : undefined, referrerPolicy: "strict-origin-when-cross-origin", src: imageBuilder
                .image(mediaProp)
...
//# sourceMappingURL=SanityDefaultPreview.js.map