...
describe('resolveInitialValue', () => {
    ...
    test('throws on missing template `value` prop', () => {
        expect(resolveInitialValue(schema, omit(example, ['value']), {}, mockConfigContext)).rejects.toMatchObject({
            message: 'Template "author" has invalid "value" property',
        });
    });
    test('throws on non-function/non-object template `value` prop', () => {
        expect(resolveInitialValue(schema, { ...example, value: [] }, {}, mockConfigContext)).rejects.toMatchObject({
            message: 'Template "author" has invalid "value" property - must be a plain object or a resolver function returning a plain object',
        });
    });
    test('throws on wrong `_type`  prop', () => {
        expect(resolveInitialValue(schema, { ...example, value: { _type: 'foo' } }, {}, mockConfigContext)).rejects.toMatchObject({
            message: 'Template "author" initial value: includes "_type"-property (foo) that does not match template (author)',
        });
    });
    test('should call sync value resolvers', () => {
        expect(resolveInitialValue(schema, { ...example, value: () => example.value }, {}, mockConfigContext)).resolves.toMatchObject({
            title: 'here',
        });
    });
    test('should call async value resolvers', () => {
        expect(resolveInitialValue(schema, {
            ...example,
            value: () => Promise.resolve(example.value),
        }, {}, mockConfigContext)).resolves.toMatchObject({
            title: 'here',
        });
    });
    test('throws on wrong value type resolved', () => {
        expect(resolveInitialValue(schema, { ...example, value: () => null }, {}, mockConfigContext)).rejects.toMatchObject({
            message: 'Template "author" has invalid "value" property - must be a plain object or a resolver function returning a plain object',
        });
    });
    ...
    test.skip('throws on values with sub-objects missing `_type`', () => {
        ...
        }, {}, mockConfigContext)).rejects.toMatchObject({
    ...
    test('applies missing `_type` to references', () => {
        ...
        }, {}, mockConfigContext)).resolves.toMatchObject({ image: { _type: 'image', asset: { _ref: 'foo', _type: 'reference' } } });
    });
