...
import type { AssetSource, CurrentUser, ObjectSchemaType, SanityDocumentLike, Schema, SchemaType, SchemaTypeDefinition } from '@sanity/types';
import type React from 'react';
import { ComponentType, ReactNode } from 'react';
import type { Observable } from 'rxjs';
import type { BifurClient } from '@sanity/bifur-client';
import type { FieldProps, FormBuilderArrayFunctionComponent, FormBuilderCustomMarkersComponent, FormBuilderMarkersComponent, InputProps, ItemProps, RenderFieldCallback, RenderInputCallback, RenderItemCallback, RenderPreviewCallback } from '../form';
...
export interface SanityFormConfig {
    ...
    components?: Record<string, ComponentType<InputProps> | {
    ...
    renderPreview?: (props: PreviewProps & {
        schemaType: SchemaType;
    }, next: RenderPreviewCallback) => ReactNode;
}
...
export declare type AsyncComposableOption<TValue, TContext> = (prev: TValue, context: TContext) => Promise<TValue>;
...
export interface SchemaPluginOptions {
    name?: string;
    types?: SchemaTypeDefinition[] | ComposableOption<SchemaTypeDefinition[], Omit<ConfigContext, 'schema' | 'currentUser' | 'getClient' | 'client'>>;
    templates?: Template[] | TemplateResolver;
}
export declare type NewDocumentOptionsResolver = ComposableOption<TemplateResponse[], NewDocumentOptionsContext>;
 ...
 * @internal
 */
export declare type _DocumentLanguageFilterComponent = ComponentType<{
    schemaType: ObjectSchemaType;
}>;
/**
 * @internal
 */
export declare type _DocumentLanguageFilterResolver = ComposableOption<_DocumentLanguageFilterComponent[], _DocumentLanguageFilterContext>;
export declare type DocumentActionsResolver = ComposableOption<DocumentActionComponent[], DocumentActionsContext>;
export declare type DocumentBadgesResolver = ComposableOption<DocumentBadgeComponent[], DocumentBadgesContext>;
...
export declare type ConfigPropertyReducer<TValue, TContext> = (prev: TValue, config: PluginOptions, context: TContext) => TValue;
export declare type AsyncConfigPropertyReducer<TValue, TContext> = (prev: TValue, config: PluginOptions, context: TContext) => TValue | Promise<TValue>;
export declare type Plugin<TOptions = void> = (options: TOptions) => PluginOptions;
...
export declare type PartialContext<TContext extends ConfigContext> = Pick<TContext, Exclude<keyof TContext, keyof ConfigContext>>;
/**
 * @public
 */
...
export interface Source {
    ...
    document: {
        ...
        unstable_languageFilter: (props: PartialContext<_DocumentLanguageFilterContext>) => _DocumentLanguageFilterComponent[];
    };
    form: {
        file: {
