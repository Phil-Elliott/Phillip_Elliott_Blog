...
export function prepareConfig(config) {
    const workspaceOptions = Array.isArray(config)
        ? config
        : [config];
    try {
    ...
    const workspaces = workspaceOptions.map(({ unstable_sources: nestedSources = [], ...rootSource }) => {
        ...
        const resolvedSources = sources.map((source) => {
            ...
            const schemaErrors = schemaValidationProblemGroups?.filter((msg) => msg.problems.some((p) => p.severity === 'error'));
            ...
            const source$ = auth.state.pipe(map(({ client, authenticated, currentUser }) => {
...
function resolveSource({ config, client, currentUser, schema, authenticated, auth, }) {
    ...
    const resolveNewDocumentOptions = (creationContext) => {
        ...
        return (templateResponses
            ...
            if (!schemaType) {
                throw new Error(`Could not find matching schema type \`${template.schemaType}\` for template \`${template.id}\``);
            }
...
//# sourceMappingURL=prepareConfig.js.map