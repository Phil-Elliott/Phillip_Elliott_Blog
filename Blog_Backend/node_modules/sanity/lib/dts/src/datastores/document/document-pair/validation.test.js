import { concat, timer, Subject, of } from 'rxjs';
...
function createSubscription(client, documentPreviewStore) {
    ...
    const stream = validation({ client, getClient, documentPreviewStore, schema }, { publishedId: 'example-id', draftId: 'drafts.example-id' }, 'movie').pipe(publish());
    stream.connect();
...
describe('validation', () => {
    ...
    it('runs `editState` through `validateDocument` to create a stream of validation statuses', async () => {
        ...
        const mockEditStateSubject = new Subject();
    ...
    it.skip('re-runs validation when dependency events change', async () => {
        ...
        const mockEditStateSubject = new Subject();
        mockEditState.mockImplementation(() => mockEditStateSubject.asObservable());
        const { subscription, closeSubscription, doneValidating } = createSubscription(client, mockPreviewStore);
        mockPreviewStore.unstable_observeDocumentPairAvailability = () => concat(of({ published: { available: true } }), subject.pipe(mapTo({ published: { available: false } })));
        // simulate first emission from validation listener
        mockEditStateSubject.next({
            id: 'example-id',
            draft: {
        ...
        await doneValidating();
        mockPreviewStore.unstable_observeDocumentPairAvailability = (id) => id === 'example-ref-id'
    ...
    it.skip('replays the last known version via `memoize` and `publishReplay`', async () => {
        ...
        const mockEditStateSubject = new Subject();
        mockEditState.mockImplementation(() => mockEditStateSubject.asObservable());
        const subscription = validation({ client, schema }, { publishedId: 'example-id', draftId: 'drafts.example-id' }, 'movie')
        ...
        const immediatePlayback = await validation({ client, schema }, { publishedId: 'example-id', draftId: 'drafts.example-id' }, 'movie')
            .pipe(first())
            .toPromise();
        const immediatePlaybackAgain = await validation({ client, schema }, { publishedId: 'example-id', draftId: 'drafts.example-id' }, 'movie')
            .pipe(first())
            .toPromise();
