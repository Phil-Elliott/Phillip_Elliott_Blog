...
const toSnapshotEvent = (document) => ({ type: 'snapshot', document });
const getDocument = (event) => event.document;
...
export const createObservableBufferedDocument = (listenerEvent$, commitMutations) => {
    ...
    const createInitialBufferedDocument = (initialSnapshot) => {
        ...
        bufferedDocument.commitHandler = (opts) => {
            ...
            commitMutations(mutation).then(opts.success, (error) => {
                const isBadRequest = 'statusCode' in error &&
    ...
    const currentBufferedDocument$ = listenerEvent$.pipe(scan((bufferedDocument, listenerEvent) => {
        ...
        return bufferedDocument;
    }, null), distinctUntilChanged(), publishReplay(1), refCount());
    ...
    const snapshotAfterSync$ = listenerEvent$.pipe(filter((ev) => ev.type === 'mutation'), withLatestFrom(currentBufferedDocument$), map(([mutationEvent, bufferedDocument]) => {
    ...
    const actionHandler$ = actions$.pipe(withLatestFrom(currentBufferedDocument$), tap(([action, bufferedDocument]) => {
...
//# sourceMappingURL=createObservableBufferedDocument.js.map