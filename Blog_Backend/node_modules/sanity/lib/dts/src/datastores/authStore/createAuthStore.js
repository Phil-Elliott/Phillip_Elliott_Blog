...
const saveToken = ({ token, projectId }) => {
    try {
        storage.setItem(getStorageKey(projectId), JSON.stringify({ token, time: new Date().toISOString() }));
    }
    catch (err) {
        console.error(err);
    }
 ...
 * @internal
 */
export function _createAuthStore({ projectId, dataset, loginMethod = 'dual', ...providerOptions }) {
    ...
    const { broadcast, messages } = createBroadcastChannel(`dual_mode_auth_${projectId}`);
