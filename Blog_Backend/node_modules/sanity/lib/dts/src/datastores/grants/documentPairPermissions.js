...
function getPairPermissions({ grantsStore, permission, draft, published, liveEdit, }) {
        ...
        case 'unpublish': {
            ...
            return [
                ...
                [
                    ...
                    checkDocumentPermission('create', published && { ...published, _id: getDraftId(published._id) }),
                ],
            ];
        }
        case 'update': {
 ...
 */
export function getDocumentPairPermissions({ client, grantsStore, schema, id, permission, type, }) {
    ...
    return snapshotPair(client, { draftId: getDraftId(id), publishedId: getPublishedId(id) }, type).pipe(switchMap((pair) => combineLatest([pair.draft.snapshots$, pair.published.snapshots$]).pipe(map(([draft, published]) => ({ draft, published })))), switchMap(({ draft, published }) => {
        const pairPermissions = getPairPermissions({
            ...
            published,
            liveEdit,
        }).map(([label, observable]) => observable.pipe(map(({ granted, reason }) => ({
            granted,
...
export function useDocumentPairPermissions({ id, type, permission, ...rest }) {
...
//# sourceMappingURL=documentPairPermissions.js.map