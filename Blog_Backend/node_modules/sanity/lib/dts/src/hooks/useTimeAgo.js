...
export function useTimeAgo(time, { minimal, agoSuffix } = {}) {
    ...
    useEffect(() => {
        const id = Number.isFinite(resolved.refreshInterval)
            ? window.setInterval(() => setResolved(formatRelativeTime(time, { minimal, agoSuffix })), resolved.refreshInterval)
            : undefined;
